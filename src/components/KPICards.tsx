    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-xl space-y-2">
        <div className="flex items-center justify-between">
          <span className="text-zinc-500 text-sm font-medium">Total Addressable Market</span>
          <div className="p-2 bg-indigo-500/10 rounded-lg">
            <DollarSign className="w-5 h-5 text-indigo-400" />
          </div>
        </div>
        <div className="text-2xl font-bold text-white">{formatCurrency(results.tam)}</div>
        <div className="text-xs text-zinc-500">Estimated annual spending capacity</div>
      </div>

      <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-xl space-y-2">
        <div className="flex items-center justify-between">
          <span className="text-zinc-500 text-sm font-medium">Target Demographic</span>
          <div className="p-2 bg-sky-500/10 rounded-lg">
            <Users className="w-5 h-5 text-sky-400" />
          </div>
        </div>
        <div className="text-2xl font-bold text-white">{formatNumber(results.targetDemographicCount)}</div>
        <div className="text-xs text-zinc-500">Size of primary target audience</div>
      </div>

      <div className={`border p-6 rounded-xl space-y-2 ${getScoreColor(results.viabilityScore)}`}>
        <div className="flex items-center justify-between">
