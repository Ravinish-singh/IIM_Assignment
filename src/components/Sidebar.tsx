...
import { LayoutDashboard, MapPin, Users, TrendingUp, Target, BarChart3, Percent } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ data, setData }) => {
  ...
  return (
    <aside className="w-80 bg-zinc-950 border-r border-zinc-800 h-screen overflow-y-auto p-6 flex flex-col gap-8 text-zinc-300">
      <div className="flex items-center gap-3 mb-2">
        <div className="p-2 bg-indigo-600 rounded-lg">
          <LayoutDashboard className="w-6 h-6 text-white" />
        </div>
        <h1 className="text-xl font-bold text-white tracking-tight">Retail Opportunity</h1>
      </div>
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full bg-zinc-900 border border-zinc-800 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
            />
          </div>
          <div className="space-y-2">
            <label className="text-xs text-zinc-500">Target Category</label>
            <select
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <div className="flex justify-between text-xs">
              <label className="text-zinc-500">Total Population</label>
              <span className="text-indigo-400 font-medium">{(data.totalPopulation / 1000).toFixed(0)}k</span>
            </div>
            <input
              type="range"
              min="10000"
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-xs">
              <label className="text-zinc-500">Annual Growth (%)</label>
              <span className="text-indigo-400 font-medium">{data.annualGrowthRate}%</span>
            </div>
            <input
              type="range"
              min="-5"
        ...
        <div className="grid grid-cols-2 gap-4">
          {(['genZ', 'millennials', 'genX', 'boomers'] as const).map((age) => (
            <div key={age} className="space-y-1">
              <label className="text-[10px] text-zinc-500 uppercase">{age}</label>
              <input
                type="number"
      ...
      <section className="mt-auto pt-6 border-t border-zinc-800 space-y-4">
        <div className="space-y-2">
          <div className="flex justify-between text-xs items-center">
            <label className="text-zinc-500 font-semibold uppercase tracking-wider">Competition</label>
            <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
              data.competitionIndex > 0.7 ? 'bg-red-500/20 text-red-400' : 
              data.competitionIndex > 0.3 ? 'bg-yellow-500/20 text-yellow-400' : 
          ...
          <input
            ...
            className="w-full accent-indigo-500 h-1.5 bg-zinc-800 rounded-lg appearance-none cursor-pointer hover:accent-indigo-400 transition-all"
          />
          <p className="text-[10px] text-zinc-600 italic">Adjusts viability based on existing market density.</p>
        </div>
      </section>
    </aside>
  );
