...
export const OpportunityList: React.FC<OpportunityListProps> = ({ category }) => {
  const items = OPPORTUNITIES[category];

  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden flex flex-col h-full max-h-[400px]">
      <div className="p-5 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/50">
        <div className="flex items-center gap-2">
          <Briefcase size={18} className="text-indigo-400" />
          <h3 className="text-white font-semibold text-sm">Key Opportunities</h3>
        </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
        {items.map((item, index) => (
          <div 
            key={item.name} 
            className="group flex items-center justify-between p-3 rounded-lg hover:bg-zinc-800/50 transition-all border border-transparent hover:border-zinc-700/50"
          >
            ...
            <div className="p-1.5 rounded-md bg-zinc-800 group-hover:bg-indigo-600 transition-colors">
      ...
      <div className="p-4 bg-zinc-950/30 border-t border-zinc-800 flex items-center gap-2">
        <Zap size={14} className="text-yellow-500" />
        <span className="text-[10px] text-zinc-500 font-medium">Market Fit Score based on local demographics.</span>
      </div>
    </div>
  );
};
